...
import { addContactToListService, sendOrderService, validateAddressService } from "@logic/services";
...
export default function CheckoutContent() {
	...
	sendGAAddToCart({
		...
		items: [
			{
				item_id: "1134",
				item_name: "Super Hair Serum | 1 Month Subscription + FREE Scalp Massager (Free Trial)",
				quantity: 1,
			},
		],
	});
	...
	const handleOrder = async () => {
		...
		if (!(customerFormRef.current && customerFormRef.current.validateFields())) isValid = false;
		if (!(deliveryFormRef.current && deliveryFormRef.current.validateFields())) isValid = false;
		if (!isBilling && !(billingAddressFormRef.current && billingAddressFormRef.current.validateFields()))
			isValid = false;
		if (!(billingFormRef.current && billingFormRef.current.validateFields())) isValid = false;
		...
		if (isValid) {
			...
			const billingFormData = billingAddressFormRef.current?.getFormData();
			const cardsFormData = billingFormRef.current?.getFormData();

			if (contactFormData && deliveryFormData && cardsFormData) {
				...
				//send to sendlane
				const dataToSendSendlane = {
					email: String(contactFormData["email"]!.value),
					firstName: String(contactFormData["firstName"]!.value),
					lastName: String(contactFormData["lastName"]!.value),
					phone: `+1${String(contactFormData["phone"]!.value).replace(/\D/g, "")}`,
					sendlaneListId: SENDLANE_LIST_ID,
				};

				const addContactdata = await evaluateFetchPost(addContactToListService, dataToSendSendlane);
				...
				//send to konnective
				const dataToSendKonnective = {
					//product data
					product1_qty: "1",
					campaignId: String(CAMPAING_ID),
					product1_id: String(productId),

					//variable to indicate if billing addres is the same of shipment address
					billShipSame: isBilling,

					//aditional user data
					emailAddress: String(contactFormData["email"]!.value),
					phoneNumber: String(contactFormData["phone"]!.value).replace(/\D/g, ""),
					firstName: String(contactFormData["firstName"]!.value),
					lastName: String(contactFormData["lastName"]!.value),

					//billing address
					address1: String(userData["address"]!.value),
					address2: String(userData["address2"]!.value),
					...
					//shipment data when billing addres is false
					shipFirstName: String(contactFormData["firstName"]!.value),
					shipLastName: String(contactFormData["lastName"]!.value),

					shipAddress1: String(deliveryFormData["address"]!.value),
					shipAddress2: String(deliveryFormData["address2"]!.value),
					...
					//FOR 3DS purposes
					redirectsTo: "https://noorhairoffers.com/api/konnective/tools/handleValidate3DS",
					errorRedirectsTo: window.location.origin + BASE_PATH_DYNAMIC_CHECKOUT,
				};

				const checkoutMETA = {
					email: dataToSendKonnective.emailAddress,
					phone: `1${dataToSendKonnective.phoneNumber}`,
					nombre: dataToSendKonnective.firstName,
					apellido: dataToSendKonnective.lastName,
					ciudad: dataToSendKonnective.city,
					estado: dataToSendKonnective.state,
					codigoPostal: dataToSendKonnective.postalCode,
					pais: dataToSendKonnective.country,
				...
				await sendMetaEvent(META_INITCHECKOUT_EVENT, checkoutMETA);

				sessionStorage.setItem("metaPurchaseObjNoor", JSON.stringify(checkoutMETA));
				sessionStorage.setItem("GAPurchaseObjNoor", JSON.stringify(checkoutGA));

				const data = await evaluateFetchPost(sendOrderService, dataToSendKonnective);
				...
				if (data) {
					if (data.items && data.orderId) {
						...
						await sendMetaEvent(META_PURCHASE_EVENT, dataMETA);
	...
	const handleZipCodeChangeDelivery = (event?: React.ChangeEvent<HTMLInputElement>) => {
		if (event) {
			const valor = event.target.value;
			let soloNumeros = valor.replace(/\D/g, "");
			const longitud = soloNumeros.length;
	...
	useEffect(() => {
		const error3DS = url.get("errorMsg");
		if (error3DS) {
			alert("Error, order not generated. Card rejected, please try a new purchase with a different card");
		}

		const orderId = url.get("orderId");
		if (orderId) {
			location.href = window.location.origin + BASE_PATH_DYNAMIC + "/us1?orderId=" + orderId;
		}
	}, []);
	...
	return (
		...
		>
			...
			<div className={styles.left_section}>
				<div className={styles.form_container}>
					<h1 className={styles.title}>
						<span>STEP 1: </span>CUSTOMER INFORMATION
					</h1>
					<Form
						ref={customerFormRef}
						...
						formConfig={[
							...
							},
							{
								type: "checkbox",
								identifier: "check",
								text: "I would like to receive order confirmation text messages and updates",
								isRequired: true,
								initialValue: true,
							},
						]}
				...
				<div className={styles.form_container}>
					<h1 className={styles.title}>
						<span>STEP 2: </span>DELIVERY ADDRESS
					</h1>
					<Form
						ref={deliveryFormRef}
						containerClassName={styles.delivery_form_container}
						formConfig={[
							{ type: "input", identifier: "address", topText: "Full address..." },
							{ type: "input", identifier: "address2", topText: "Apt/Suite...", isRequired: false },
							{ type: "input", identifier: "city", topText: "City name..." },
							{ type: "select", identifier: "state", text: "State", selectOptions: USA_STATES },
							{
								type: "input",
								identifier: "zipCode",
								topText: "Zip code...",
								onInputChange: handleZipCodeChangeDelivery,
							},
							{
								type: "select",
								identifier: "country",
				...
				<div className={styles.form_container}>
					<h1 className={styles.title}>
						<span>STEP 3: </span>BILLING ADDRESS
					</h1>
					<Checkbox
			...
			<div className={styles.right_section}>
				<div className={styles.form_container}>
					<h1 className={styles.title}>
						<span>STEP 4: </span>BILLING INFORMATION
					</h1>
					<div className={styles.billing_form_container}>
